...
const Bytes = Buffer.from("5d019aa9ff5f77ae12458a14f161a9ac7c49b8cb41959f29e20f29811d47b81f", "hex");
const Sha256Bytes = Buffer.from("7196633b1ce5bf5fa369169ea81fc03a8238b38a888ef226a1e1bb42ab186bcb", "hex");
const Sha1Bytes = Buffer.from("a88a129b1afa94547623f27d7c6e566a7d902f7c", "hex");
const Ripemd160Bytes = Buffer.from("cbf33f6b1a8ddd0ba7960f639d703653d111b366", "hex");
const PublicKeyHash = Buffer.from("67e403c30d6a2f5240bdd47b4d977c2c006f9639e80ccaf24547569f03729f60", "hex");
const PublicKeys = [
    Buffer.from("0209087ba51d402d3f03fe6437ff273d972b3bba3659b6a0735a440c71059127c0", "hex"),
    Buffer.from("02e997d0f14c8adbb0637c5f8ab803cdfcfefc257c8e4b85c2d770d64b5b8f88f0", "hex"),
    Buffer.from("039a43418eb28a74b42dc8e94716ed1b0967276bd69ab7fe51647c58b762621f62", "hex")
];
const PrivateKeys = [
...
exports.TEST_CONTRACT_CLAUSE_NAMES = {
...
exports.TEST_CONTRACT_TIMES = {
    LockUntil: 20
};
exports.TEST_CONTRACT_AGES = {
    LockDelay: 20,
    VaultSpend: 20,
    EscrowWithDelay: 20
};
...
exports.TEST_CASES = Object.assign({}, exports.DEMO_CONTRACTS, { HashOperations: `contract HashOperations(
  hash1: Sha256(Bytes),
  hash2: Sha1(Bytes),
...
function generateSignature(id, privateKeyIndex) {
    const privateKey = PrivateKeys[privateKeyIndex];
    const template = compile_1.compile(exports.TEST_CASES[id]);
    const instantiated = instantiate_1.instantiate(template, exports.TEST_CONTRACT_ARGS[id], seed);
    const tx = spend_1.spend(instantiated.fundingTransaction, destinationAddress, 0, exports.TEST_CONTRACT_TIMES[id] || 0, { sequence: exports.TEST_CONTRACT_AGES[id] || 0, seconds: false });
    const sigHash = spend_2.toSighash(instantiated, tx);
    if (sigHash === undefined) {
        throw new Error("sighash unexpectedly undefined");
    }
    const sig = spend_2.createSignature(sigHash, privateKey);
    if (sig === undefined) {
