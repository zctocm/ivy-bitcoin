...
ConfirmStats.prototype.init = function init(buckets, maxConfirms, decay) {
  ...
  for (var _i = 0; _i < maxConfirms; _i++) {
    this.confAvg[_i] = new Float64Array(buckets.length);
    this.curBlockConf[_i] = new Int32Array(buckets.length);
    this.unconfTX[_i] = new Int32Array(buckets.length);
  }
...
ConfirmStats.prototype.estimateMedian = function estimateMedian(target, needed, breakpoint, greater, height) {
  ...
  for (var _i2 = minBucket; _i2 <= maxBucket; _i2++) {
    sum += this.txAvg[_i2];
  }if (found && sum !== 0) {
    sum = sum / 2;
    for (var _j = minBucket; _j <= maxBucket; _j++) {
      if (this.txAvg[_j] < sum) {
        sum -= this.txAvg[_j];
      } else {
        median = this.avg[_j] / this.txAvg[_j];
        break;
      }
    }
  }
...
ConfirmStats.prototype.fromRaw = function fromRaw(data) {
  ...
  for (var _i3 = 0; _i3 < maxConfirms; _i3++) {
    if (confAvg[_i3].length !== buckets.length) throw new Error('Mismatch in fee/pri conf average bucket count.');
  }
...
PolicyEstimator.prototype.init = function init() {
  ...
  for (var _b = minPri; _b <= MAX_PRIORITY; _b *= PRI_SPACING) {
    priority.push(_b);
...
PolicyEstimator.prototype.processTX = function processTX(entry, current) {
  ...
  if (fee === 0 || this.isPriPoint(rate, priority)) {
    ...
    var _item = new StatEntry();
    _item.blockHeight = height;
    _item.bucketIndex = this.feeStats.addTX(height, rate);
    this.map.set(hash, _item);
  } else {
...
PolicyEstimator.prototype.estimateFee = function estimateFee(target, smart) {
  ...
  if (!smart) {
    var _rate = this.feeStats.estimateMedian(target, SUFFICIENT_FEETXS, MIN_SUCCESS_PCT, true, this.bestHeight);

    if (_rate < 0) return 0;

    return Math.floor(_rate);
  }
...
PolicyEstimator.prototype.estimatePriority = function estimatePriority(target, smart) {
  ...
  if (!smart) {
    var _priority = this.priStats.estimateMedian(target, SUFFICIENT_PRITXS, MIN_SUCCESS_PCT, true, this.bestHeight);
    return Math.floor(_priority);
  }
