...
RequestOptions.prototype.isOverflow = function isOverflow(hdr) {
  ...
  var length = parseInt(hdr, 10);

  if (!isFinite(length)) return true;
...
RequestOptions.prototype.getHeaders = function getHeaders() {
  ...
  if (this.auth) {
    var auth = this.auth.username + ':' + this.auth.password;
    var data = Buffer.from(auth, 'utf8');
...
(0, _setPrototypeOf2.default)(Request.prototype, Stream.prototype);
...
Request.prototype.startTimeout = function startTimeout() {
  ...
  this.timeout = setTimeout(function () {
    _this.finish(new Error('Request timed out.'));
...
Request.prototype.cleanup = function cleanup() {
  ...
  if (this.request) {
    ...
    this.request.addListener('error', function () {});
  }
  ...
  if (this.response) {
    ...
    this.response.addListener('error', function () {});
  }
};
...
Request.prototype._onResponse = function _onResponse(response) {
  ...
  var type = parseType(contentType);
