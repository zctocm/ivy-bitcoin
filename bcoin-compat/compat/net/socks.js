...
(0, _setPrototypeOf2.default)(SOCKS.prototype, EventEmitter.prototype);
...
SOCKS.prototype.startTimeout = function startTimeout() {
  ...
  this.timeout = setTimeout(function () {
    var state = SOCKS.statesByVal[_this.state];
    _this.timeout = null;
    _this.error('SOCKS request timed out (state=%s).', state);
  }, 8000);
};
...
SOCKS.prototype.handleClose = function handleClose() {
  if (this.state !== this.target) {
    var state = SOCKS.statesByVal[this.state];
...
SOCKS.prototype.sendAuth = function sendAuth() {
  ...
  var ulen = Buffer.byteLength(user, 'ascii');
  var plen = Buffer.byteLength(pass, 'ascii');
  var size = 3 + ulen + plen;
...
SOCKS.prototype.sendProxy = function sendProxy() {
  ...
  if (type === 0x03) bw.writeU8(name.length);
...
(0, _setPrototypeOf2.default)(Proxy.prototype, EventEmitter.prototype);
...
Proxy.prototype.connect = function () {
  var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(port, host) {
    ...
    var options, socket, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, op;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 13:
            ...
            _context.prev = 24;
            for (_iterator = (0, _getIterator3.default)(this.ops); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              op = _step.value;
    ...
    }, _callee, this, [[3, 9], [24, 28, 32, 40], [33,, 35, 39]]);
  }));
...
Proxy.prototype.setKeepAlive = function setKeepAlive(enable, delay) {
  ...
  if (!this.socket) {
    this.ops.push(function () {
      _this4.socket.setKeepAlive(enable, delay);
    });
    return;
  }
...
Proxy.prototype.setNoDelay = function setNoDelay(enable) {
  ...
  if (!this.socket) {
    this.ops.push(function () {
      _this5.socket.setNoDelay(enable);
    });
    return;
  }
...
Proxy.prototype.setTimeout = function setTimeout(timeout, callback) {
  ...
  if (!this.socket) {
    this.ops.push(function () {
      _this6.socket.setTimeout(timeout, callback);
    });
    return;
  }
...
function parseProxy(host) {
  ...
  if (index === -1) {
    var _addr = IP.fromHostname(host, 1080);
    return {
      host: _addr.host,
      port: _addr.port
    };
  ...
  var left = host.substring(0, index);
  var right = host.substring(index + 1);

  var parts = left.split(':');
  assert(parts.length > 1, 'Bad username and password.');

  var addr = IP.fromHostname(right, 1080);
...
exports.connect = function connect(proxy, destPort, destHost) {
  ...
  var socket = new Proxy(host, port, user, pass);
