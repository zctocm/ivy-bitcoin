...
KeyRing.prototype.fromOptions = function fromOptions(options, network) {
  ...
  if (script) return this.fromScript(key, script, compress, network);
...
KeyRing.prototype.fromKey = function fromKey(key, compress, network) {
  ...
  if (key.length === 32) return this.fromPrivate(key, compress !== false, network);
...
KeyRing.prototype.getProgram = function getProgram() {
  ...
  if (!this._program) {
    var program = void 0;
    if (!this.script) {
      var hash = digest.hash160(this.publicKey);
      program = Script.fromProgram(0, hash);
    } else {
      var _hash = this.script.sha256();
      program = Script.fromProgram(0, _hash);
    }
...
KeyRing.prototype.getNestedAddress = function getNestedAddress(enc) {
  ...
  if (!this._nestedAddress) {
    ...
    var addr = Address.fromScripthash(hash, this.network);
...
KeyRing.prototype.getScriptAddress = function getScriptAddress(enc) {
  ...
  if (!this._scriptAddress) {
    ...
    if (this.witness) {
      ...
      var _hash2 = this.getScriptHash160();
      addr = Address.fromScripthash(_hash2, this.network);
    }
...
KeyRing.prototype.getKeyHash = function getKeyHash(enc) {
  if (!this._keyHash) this._keyHash = digest.hash160(this.publicKey);
...
KeyRing.prototype.getKeyAddress = function getKeyAddress(enc) {
  if (!this._keyAddress) {
    ...
    var addr = void 0;
    if (this.witness) addr = Address.fromWitnessPubkeyhash(hash, this.network);else addr = Address.fromPubkeyhash(hash, this.network);
