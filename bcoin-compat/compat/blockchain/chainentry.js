...
function ChainEntry(options) {
  if (!(this instanceof ChainEntry)) return new ChainEntry(options);
...
ChainEntry.prototype.hasUnknown = function hasUnknown(network) {
  var TOP_MASK = consensus.VERSION_TOP_MASK;
  var TOP_BITS = consensus.VERSION_TOP_BITS;
  var bits = (this.version & TOP_MASK) >>> 0;

  if (bits !== TOP_BITS) return false;
