...
var wordlistCache = (0, _create2.default)(null);
...
Mnemonic.prototype.toSeed = function toSeed(passphrase) {
  ...
  var passwd = nfkd('mnemonic' + passphrase);

  return pbkdf2.derive(Buffer.from(phrase, 'utf8'), Buffer.from(passwd, 'utf8'), 2048, 64, 'sha512');
};
...
Mnemonic.prototype.fromPhrase = function fromPhrase(phrase) {
  ...
  var chk2 = digest.sha256(entropy);

  // Verify checksum.
  for (var _i = 0; _i < cbits; _i++) {
    var _bit = _i % 8;
    var _oct = (_i - _bit) / 8;
    var b1 = chk1[_oct] >>> 7 - _bit & 1;
    var b2 = chk2[_oct] >>> 7 - _bit & 1;
...
Mnemonic.getLanguage = function getLanguage(word) {
  ...
  try {
    for (var _iterator = (0, _getIterator3.default)(Mnemonic.languages), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
