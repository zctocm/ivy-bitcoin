...
HDPrivateKey.prototype.derive = function derive(index, hardened) {
  ...
  var hash = digest.hmac('sha512', data, this.chainCode);
  ...
  if (this.fingerPrint === -1) {
    var fp = digest.hash160(this.publicKey);
...
HDPrivateKey.prototype.deriveAccount = function deriveAccount(purpose, account) {
  ...
  return this.derive(purpose, true).derive(this.network.keyPrefix.coinType, true).derive(account, true);
};
...
HDPrivateKey.prototype.equals = function equals(obj) {
  ...
  return this.network === obj.network && this.depth === obj.depth && this.parentFingerPrint === obj.parentFingerPrint && this.childIndex === obj.childIndex && this.chainCode.equals(obj.chainCode) && this.privateKey.equals(obj.privateKey);
};
...
HDPrivateKey.prototype.fromSeed = function fromSeed(seed, network) {
  ...
  var hash = digest.hmac('sha512', seed, SEED_SALT);
  var left = hash.slice(0, 32);
  var right = hash.slice(32, 64);
