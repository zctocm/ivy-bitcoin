...
Opcode.prototype.toSymbol = function toSymbol() {
  ...
  if (!symbol) return '0x' + util.hex8(this.value);
...
Opcode.prototype.toFormat = function toFormat() {
  ...
  if (this.data) {
    ...
    // Direct push
    if (!_symbol) {
      var _size = util.hex8(this.value);
  ...
  // Unknown opcodes
  var value = util.hex8(this.value);
...
Opcode.fromInt = function fromInt(num) {
  ...
  if (num === 0) return Opcode.fromOp(opcodes.OP_0);

  if (num === -1) return Opcode.fromOp(opcodes.OP_1NEGATE);

  if (num >= 1 && num <= 16) return Opcode.fromOp(num + 0x50);
...
Opcode.fromSymbol = function fromSymbol(name) {
  ...
  if (op != null) return Opcode.fromOp(op);
    ...
    case opcodes.OP_PUSHDATA4:
      {
        ...
        return new Opcode(value, _data2);
      }
    default:
      {
        if (br.left() < value) {
        ...
        return new Opcode(value, _data3);
      }
  }
};
...
PARSE_ERROR = (0, _freeze2.default)(new Opcode(-1));
...
for (var value = 0x00; value <= 0xff; value++) {
  ...
  opCache.push((0, _freeze2.default)(op));
}
