...
var _require = require('../crypto/scrypt'),
    derive = _require.derive;

var hashcash = require('../mining/mine');
...
jobs.checkInput = function checkInput(tx, index, coin, flags) {
  try {
    ...
    if (err.type === 'ScriptError') return new packets.CheckInputResultPacket(err);
    throw err;
  }
...
jobs.mine = function mine(data, target, min, max) {
  var nonce = hashcash(data, target, min, max);
