...
AESKey.prototype.destroy = function destroy() {
  ...
  if (this.decryptKey) {
    for (var i = 0; i < this.decryptKey.length; i++) {
      this.decryptKey[i] = 0;
    }this.decryptKey = null;
  }

  if (this.encryptKey) {
    for (var _i = 0; _i < this.encryptKey.length; _i++) {
...
AESKey.prototype.getEncryptKey = function getEncryptKey() {
  ...
  if (this.bits === 192) {
    for (;;) {
      ...
      key[kp + 6] = key[kp + 0] ^ TE2[_tmp >>> 16 & 0xff] & 0xff000000 ^ TE3[_tmp >>> 8 & 0xff] & 0x00ff0000 ^ TE0[_tmp >>> 0 & 0xff] & 0x0000ff00 ^ TE1[_tmp >>> 24 & 0xff] & 0x000000ff ^ RCON[i];
  ...
  if (this.bits === 256) {
    for (;;) {
      ...
      key[kp + 8] = key[kp + 0] ^ TE2[_tmp2 >>> 16 & 0xff] & 0xff000000 ^ TE3[_tmp2 >>> 8 & 0xff] & 0x00ff0000 ^ TE0[_tmp2 >>> 0 & 0xff] & 0x0000ff00 ^ TE1[_tmp2 >>> 24 & 0xff] & 0x000000ff ^ RCON[i];
      ...
      key[kp + 12] = key[kp + 4] ^ TE2[_tmp2 >>> 24 & 0xff] & 0xff000000 ^ TE3[_tmp2 >>> 16 & 0xff] & 0x00ff0000 ^ TE0[_tmp2 >>> 8 & 0xff] & 0x0000ff00 ^ TE1[_tmp2 >>> 0 & 0xff] & 0x000000ff;
...
AESKey.prototype.getDecryptKey = function getDecryptKey() {
  ...
  for (var _i2 = 0, j = 4 * this.rounds; _i2 < j; _i2 += 4, j -= 4) {
    var tmp = key[kp + _i2 + 0];

    key[kp + _i2 + 0] = key[kp + j + 0];
    key[kp + j + 0] = tmp;

    tmp = key[kp + _i2 + 1];
    key[kp + _i2 + 1] = key[kp + j + 1];
    key[kp + j + 1] = tmp;

    tmp = key[kp + _i2 + 2];
    key[kp + _i2 + 2] = key[kp + j + 2];
    key[kp + j + 2] = tmp;

    tmp = key[kp + _i2 + 3];
    key[kp + _i2 + 3] = key[kp + j + 3];
    key[kp + j + 3] = tmp;
  }

  // Apply the inverse MixColumn transform to
  // all round keys but the first and the last.
  for (var _i3 = 1; _i3 < this.rounds; _i3++) {
    kp += 4;
...
AESKey.prototype.encryptBlock = function encryptBlock(input) {
  ...
  var s0 = readU32(input, 0) ^ key[0];
  var s1 = readU32(input, 4) ^ key[1];
  var s2 = readU32(input, 8) ^ key[2];
  var s3 = readU32(input, 12) ^ key[3];
  ...
  var output = Buffer.allocUnsafe(16);
...
AESKey.prototype.decryptBlock = function decryptBlock(input) {
  ...
  // Map byte array block to cipher
  // state and add initial round key.
  var s0 = readU32(input, 0) ^ key[kp + 0];
  var s1 = readU32(input, 4) ^ key[kp + 1];
  var s2 = readU32(input, 8) ^ key[kp + 2];
  var s3 = readU32(input, 12) ^ key[kp + 3];
...
AESCipher.prototype.final = function final() {
  ...
  if (!this.waiting) {
    ...
    var left = 16 - this.waiting.length;
...
AESDecipher.prototype.update = function update(data) {
  ...
  for (var i = 0; i < len; i += 16) {
    ...
    var block = this.key.decryptBlock(this.prev);
