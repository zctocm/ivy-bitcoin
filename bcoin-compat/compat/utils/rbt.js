...
RBT.prototype.search = function search(key) {
  ...
  while (!current.isNull()) {
    var cmp = this.compare(key, current.key);
...
RBT.prototype.insert = function insert(key, value) {
  ...
  var node = new RBTNode(key, value);
...
RBT.prototype.removeNode = function removeNode(z) {
  ...
  var x = y.left.isNull() ? y.right : y.left;
  x.parent = y.parent;
...
RBT.prototype.removeFixup = function removeFixup(x) {
  while (x !== this.root && x.color === BLACK) {
    if (x === x.parent.left) {
      ...
      if (_w.right.color === BLACK && _w.left.color === BLACK) {
        _w.color = RED;
        x = x.parent;
      } else {
        if (_w.left.color === BLACK) {
          _w.right.color = BLACK;
          _w.color = RED;
          this.rotl(_w);
          _w = x.parent.left;
        }
        _w.color = x.parent.color;
        x.parent.color = BLACK;
        _w.left.color = BLACK;
        this.rotr(x.parent);
        x = this.root;
...
RBT.prototype.clone = function clone() {
  if (this.root.isNull()) return SENTINEL;
...
Iterator.prototype.seekMin = function seekMin(key) {
  ...
  while (!root.isNull()) {
    var cmp = this.tree.compare(root.key, key);
