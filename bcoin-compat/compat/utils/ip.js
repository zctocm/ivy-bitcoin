...
IP.fromHostname = function fromHostname(addr, fallback) {
  ...
  var type = IP.getStringType(host);
  ...
  var hostname = void 0;
  if (type === IP.types.IPV6) hostname = '[' + host + ']:' + port;else hostname = host + ':' + port;
...
IP.toHostname = function toHostname(host, port) {
  ...
  if (type === IP.types.IPV6) return '[' + host + ']:' + port;
...
IP.toBuffer = function toBuffer(str) {
  ...
  if (IP.isOnionString(str)) {
    var prefix = TOR_ONION;
    prefix.copy(raw, 0);
    var data = base32.decode(str.slice(0, -6));
...
IP.parseV6 = function parseV6(str, raw, offset) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (IP.isV4String(_word)) {
      IP.parseV4(_word, raw, offset);
      offset += 4;
      continue;
    }

    assert(_word.length <= 4);

    _word = parseInt(_word, 16);

    assert(_word === _word, 'Non-number in IPv6 address.');

    raw[offset++] = _word >> 8 & 0xff;
    raw[offset++] = _word & 0xff;
  }
...
IP.toString = function toString(raw) {
  ...
  if (raw.length === 16) {
    ...
    if (IP.isOnion(raw)) {
      var _host3 = base32.encode(raw.slice(6));
      return _host3 + '.onion';
    }

    var _host = '';

    _host += raw.readUInt16BE(0, true).toString(16);

    for (var i = 2; i < 16; i += 2) {
      _host += ':';
      _host += raw.readUInt16BE(i, true).toString(16);
    }

    _host = _host.replace(/(^|:)0(:0)*:0(:|$)/, '$1::$3');
    _host = _host.replace(/:{3,4}/, '::');
...
IP.getInterfaces = function getInterfaces(name, family) {
  ...
  try {
    for (var _iterator3 = (0, _getIterator3.default)((0, _keys2.default)(interfaces)), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      ...
      try {
        for (var _iterator4 = (0, _getIterator3.default)(items), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          var details = _step4.value;

          var type = details.family.toLowerCase();
