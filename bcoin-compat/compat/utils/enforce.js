...
function enforce(value, name, type, func) {
  if (!value) {
    if (!func) func = enforce;

    if (name && !type) throwError(name, func);
...
function throwError(msg, func) {
  ...
  if (Error.captureStackTrace && func) Error.captureStackTrace(error, func);
  throw error;
}
...
enforce.obj = function obj(v, name) {
  enforce(v && (typeof v === 'undefined' ? 'undefined' : (0, _typeof3.default)(v)) === 'object' && !Array.isArray(v), name, 'object', obj);
};
