...
Bloom.prototype.fromOptions = function fromOptions(size, n, tweak, update) {
  ...
  assert((0, _isSafeInteger2.default)(size), '`size` must be an integer.');

  size -= size % 8;

  var filter = Buffer.allocUnsafe(size / 8);
  filter.fill(0);
  ...
  assert((0, _isSafeInteger2.default)(n), '`n` must be an integer.');
  assert(typeof tweak === 'number', '`tweak` must be a number.');
  assert((0, _isSafeInteger2.default)(tweak), '`tweak` must be an integer.');
...
Bloom.fromRate = function fromRate(items, rate, update) {
  ...
  var bits = -1 / LN2SQUARED * items * Math.log(rate) | 0;
  var size = Math.max(8, bits);

  if (update !== -1) {
    assert(size <= Bloom.MAX_BLOOM_FILTER_SIZE * 8, 'Bloom filter size violates policy limits!');
  }

  var n = Math.max(1, size / items * LN2 | 0);
