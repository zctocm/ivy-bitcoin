...
AsyncEmitter.prototype.prependListener = function prependListener(
  type,
  handler
) {
...
AsyncEmitter.prototype.prependOnceListener = function prependOnceListener(
  type,
  handler
) {
...
AsyncEmitter.prototype.listeners = function listeners(type) {
  ...
  const listenersList = this._events[type];

  if (!listenersList) return [];

  const result = [];

  for (const listener of listenersList) result.push(listener.handler);
