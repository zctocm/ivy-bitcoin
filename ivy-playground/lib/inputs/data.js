import { createSignature, crypto, isHash } from "ivy-bitcoin";
import { getChild, getInputNameContext, getInputType, isPrimaryInputType } from "./types";
...
export const isValidInput = (id, inputMap) => {
...
export function isValidBTC(value) {
    return (value !== "" &&
        /^(\d+)?(\.\d{0,8})?$/.test(value) &&
    ...
    switch (input.type) {
        ...
        case "generateAddressInput":
            return (input.value === "generatePrivateKeyInput" ||
                input.value === "providePrivateKeyInput");
        case "signatureInput":
            return (input.value === "generateSignatureInput" ||
...
export function getSequence(inputsById) {
    const durationInput = inputsById["transactionDetails.sequenceNumberInput.durationInput"];
    if (durationInput.value === "secondsDurationInput") {
        const input = inputsById["transactionDetails.sequenceNumberInput.durationInput.secondsDurationInput"];
        ...
        const input = inputsById["transactionDetails.sequenceNumberInput.durationInput.blocksDurationInput"];
        if (!validateInput(input)) {
            throw new Error("invalid sequence number");
        }
        const val = input.value;
...
export function getData(inputId, inputsById, sigHash) {
...
export function getGenerateBytesInputValue(input) {
    const length = parseInt(input.value, 10);
    if (isNaN(length) || length < 0 || length > 520) {
        throw new Error("invalid length value for generateBytesInput: " + input.value);
    }
    return input.seed.slice(0, length * 2); // dumb, for now
}
function getGenerateAddressValue(inputId, inputsById) {
    const input = inputsById[inputId];
...
export function getDefaultContractParameterValue(inputType) {
    switch (inputType) {
        case "parameterInput":
        case "generateHashInput":
        case "lockTimeInput":
        ...
        case "generateAddressInput":
        case "provideAddressInput":
            throw new Error("getDefaultContractParameterValue should not be called on " + inputType);
        case "booleanInput":
            return "false";
        case "generateBytesInput":
            return "32";
...
export function getDefaultTransactionDetailValue(inputType) {
    switch (inputType) {
        case "lockTimeInput":
            return "timeInput";
        case "sequenceNumberInput":
...
export function getDefaultClauseParameterValue(inputType) {
    switch (inputType) {
        case "parameterInput":
        case "generateHashInput":
        case "addressInput":
        ...
        case "sequenceNumberInput":
        case "valueInput":
            throw new Error("getDefaultClauseParameterValue should not be called on " + inputType);
        case "booleanInput":
            return "false";
        case "generateBytesInput":
            return "32";
...
export function getDefaultValue(inputType, name) {
...
export function addDefaultInput(inputs, inputType, parentName) {
...
export function addParameterInput(inputs, valueType, name) {
...
export const getInputContext = (input) => {
    return getInputNameContext(input.name);
};
export const getParameterIdentifier = (input) => {
