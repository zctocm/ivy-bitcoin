...
import { getInputMap, getParameterIds, getParameterIds2 } from "../../templates/selectors";
...
function mapToInputProps2(pageShowError, inputsById, id) {
    const input = inputsById[id];
    if (input === undefined) {
        throw new Error("bad input ID: " + id);
    }
...
function mapStateToContractInputProps2(state, ownProps) {
...
function ContractParametersUnconnectedTwo(props) {
    if (props.parameterIds.length === 0) {
        return React.createElement("div", null);
    }
    const parameterInputs = props.parameterIds.map(id => {
        return (React.createElement("div", { key: id, className: "argument" }, getWidget2(id)));
    });
    return (React.createElement("section", { style: { wordBreak: "break-all" } },
        React.createElement(Form, null, parameterInputs)));
}
...
export const ContractParametersTwo = connect(mapStateToContractParametersPropsTwo)(ContractParametersUnconnectedTwo);
